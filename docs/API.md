...
# Use in requests
curl -H "X-API-Key: your-secret-key" https://fraud.erfi.dev/admin/...
```
...
```bash
# Summary statistics
curl -H "X-API-Key: $KEY" "https://fraud.erfi.dev/admin/analytics?type=summary&hours=24"
...
```bash
curl -X POST "https://fraud.erfi.dev/admin/analytics" \
  -H "X-API-Key: $KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -H "X-API-Key: $KEY" "https://fraud.erfi.dev/admin/analytics/queries"
```
...
```bash
curl -H "X-API-Key: $KEY" "https://fraud.erfi.dev/admin/config"
```
...
```bash
curl -X POST "https://fraud.erfi.dev/admin/markov/train" \
  -H "X-API-Key: $KEY"
```

### Dashboard

Access the analytics dashboard at: `https://fraud.erfi.dev/dashboard/`
